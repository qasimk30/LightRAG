...
from extensions.prompt_extension import PROMPTS_Extension
...
async def _merge_nodes_then_upsert(
    ...
    if already_node:
        ...
        already_screenshot.extend(
            split_string_by_multi_markers(already_node['screenshot_path'], [GRAPH_FIELD_SEP])
        )
    ...
    screenshot_path = GRAPH_FIELD_SEP.join(
        set([dp["screenshot_path"] for dp in nodes_data] + already_screenshot)
    )
...
async def _merge_edges_then_upsert(
    ...
    if await knowledge_graph_inst.has_edge(src_id, tgt_id):
        ...
        if already_edge:
            ...
            if already_edge.get("screenshot_path") is not None:
                already_screenshot.extend(
                    split_string_by_multi_markers(
                        already_edge["screenshot_path"], [GRAPH_FIELD_SEP]
                    )
                )
    ...
    screenshot_path = GRAPH_FIELD_SEP.join(
        set(
            [dp["screenshot_path"] for dp in edges_data if dp.get("screenshot_path")]
            + already_screenshot
        )
    )
        ...
        if entity_vdb is not None and entities_data:
            data_for_vdb = {
                compute_mdhash_id(dp["entity_name"], prefix="ent-"): {
                    ...
                    "screenshot_path": dp.get("screenshot_path", "None"),
                }
...
async def extract_entities(
    ...
    async def _process_extraction_result(
        result: str, chunk_key: str, file_path: str = "unknown_source", screenshot_path: str = "None",
    ):
        ...
        for record in records:
            ...
            if_entities = await _handle_single_entity_extraction(
                record_attributes, chunk_key, file_path, screenshot_path
            )
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        screenshot_path = chunk_dp.get("screenshot_path", "None")
        ...
        maybe_nodes, maybe_edges = await _process_extraction_result(
            final_result, chunk_key, file_path, screenshot_path
        )
...
async def _get_node_data(
    ...
    for i, n in enumerate(node_datas):
        ...
        screenshot_path = n.get("screenshot_path", "None")
    ...
    for i, e in enumerate(use_relations):
        ...
        screenshot_path = e.get("screenshot_path", "None")
        ...
        text_units_context.append(
            {
                ...
                "screenshot_path": t.get("screenshot_path", "unknown_source"),
            }
        )
    return entities_context, relations_context, text_units_context
...
async def _get_edge_data(
    ...
    for i, e in enumerate(edge_datas):
        ...
        # Get file path from edge data
        file_path = e.get("file_path", "unknown_source")
        screenshot_path = e.get("screenshot_path", "unknown_source")
    ...
    for i, n in enumerate(use_entities):
        ...
        # Get file path from node data
        file_path = n.get("file_path", "unknown_source")
        screenshot_path = n.get("screenshot_path", "unknown_source")
        ...
        text_units_context.append(
            {
                ...
                "screenshot_path": t.get("screenshot_path", "unknown"),
            }
        )
    return entities_context, relations_context, text_units_context
